...
namespace Force.Crc32
{
    ...
    public abstract class Crc32AlgorithmBase : HashAlgorithm
    {
        ...
        /// </summary>
        protected Settings.Base Settings
        {
            get => _settings;
            set
        ...
        internal Crc32AlgorithmBase(bool isBigEndian = true)
            : this()
        {
        ...
        {
            return Append(Settings.Initial, input, offset, length);
        }
        ...
        {
            return Append(Settings.Initial, input);
        }
        ...
        public uint ComputeAndWriteToEnd(byte[] input, int offset, int length)
        {
            ...
            var crc = Append(Settings.Initial, input, offset, length);
            var r = offset + length;
            input[r] = (byte)crc;
        ...
        {
            return Append(0, input, offset, lengthWithCrc) == Settings.Check;
        }
        ...
        public bool IsValidWithCrcAtEnd(byte[] input)
        {
            if (input.Length < 4)
                throw new ArgumentOutOfRangeException(nameof(input), "Input array should be 4 bytes at least");
            return Append(0, input, 0, input.Length) == Settings.Check;
        }
        ...
        {
            _currentCrc = Settings.Initial;
        }

        /// <inheritdoc />
        /// <summary>
        /// Appends CRC-32 from given buffer <see cref="HashCore"/>
        /// </summary>
        protected override void HashCore(byte[] array, int ibStart, int cbSize)
        {
            _currentCrc = AppendInternal(_currentCrc, array, ibStart, cbSize);
        }
